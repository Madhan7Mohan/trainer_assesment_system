...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rg-card { width:100%; max-width:480px; animation:cardIn .5s cubic-bezier(.16,1,.3,1); }
  ...
  .rg-card-sub { font-size:12px; color:#64748b; margin-bottom:28px; line-height:1.6; }
  ...
  .rg-divider { height:1px; background:rgba(148,163,184,.08); margin:20px 0; }
  ...
  .rg-submit { width:100%; padding:13px; background:linear-gradient(135deg,#00ACC1,#0891b2); border:none; border-radius:10px; font-family:'Cabinet Grotesk',sans-serif; font-size:15px; font-weight:700; color:#fff; cursor:pointer; transition:all .2s; box-shadow:0 4px 20px rgba(0,172,193,.25); margin-top:22px; }
...
export default function Register({ onRegistered, goToLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    // Save profile to public.users immediately
    const { data: prof, error: profError } = await supabase.from("users").insert([{
      id:      uid,
      name:    form.name.trim(),
      email,
    ...
    <>
      ...
      <div className="rg-root">
        <div className="rg-left">
          ...
          <p className="rg-desc">The trainer assessment platform built to evaluate real coding skills — not just theory.</p>
          <div className="rg-steps">
            {["Create your account","Log in to your account","Pick a mode and start","Get your results"].map((s,i) => (
          ...
          <div className="rg-card">
            ...
            <div className="rg-grid2">
              <div className="rg-field rg-full">
                ...
                <input className={inputCls("name")} placeholder="e.g. Ravi Kumar" value={form.name} onChange={set("name")} />
              ...
              <div className="rg-field">
                ...
                <input className={inputCls("password")} type="password" placeholder="Min 6 chars" value={form.password} onChange={set("password")} />
                {errors.password && <span className="rg-err">⚠ {errors.password}</span>}
              </div>
              <div className="rg-field">
                <label className="rg-label">Confirm Password</label>
                <input className={inputCls("confirmPassword")} type="password" placeholder="Repeat password" value={form.confirmPassword} onChange={set("confirmPassword")} />
              ...
              <div className="rg-field">
                ...
                <input className={inputCls("phone")} type="tel" placeholder="10-digit" value={form.phone} onChange={set("phone")} />
                {errors.phone && <span className="rg-err">⚠ {errors.phone}</span>}
              </div>
              <div className="rg-field">
                <label className="rg-label">Date of Birth</label>
                <input className={inputCls("dob")} type="date" value={form.dob} onChange={set("dob")} />
                {errors.dob && <span className="rg-err">⚠ {errors.dob}</span>}
              </div>
              <div className="rg-field rg-full">
                <label className="rg-label">College / Organization</label>
                <input className={inputCls("college")} placeholder="e.g. IIT Madras / ThopsTech" value={form.college} onChange={set("college")} />
